...
function F_printTestResultStat($data, $nextorderdir, $order_field, $filter, $pubmode = false, $stats = 1)
{
    ...
    $ret = '';
    $ret .= '<table class="userselect" id="test_result_users">'.K_NEWLINE;
    $ret .= '<tr>'.K_NEWLINE;
    $ret .= '<th>&nbsp;</th>'.K_NEWLINE;
    $ret .= '<th>#</th>'.K_NEWLINE;
    ...
    foreach ($data['testuser'] as $tu) {
        ...
        $ret .= '<td class="numeric"><span '.$passmsg.'>'.F_formatFloatAlt($tu['total_score']).'</span></td>'.K_NEWLINE;
        $ret .= '<td class="numeric"><span '.$passmsg.'>'.F_formatPercentageAlt($tu['total_score_perc'], false).'</span></td>'.K_NEWLINE;
        if ($stats > 0) {
