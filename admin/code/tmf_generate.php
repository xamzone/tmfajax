...
$thispage_title_icon = '<i class="fas fa-history"></i> ';
...
echo '<div class="tceformbox ta-left">'.K_NEWLINE;
echo '<form action="'.$_SERVER['SCRIPT_NAME'].'" method="post" enctype="multipart/form-data" id="form_testeditor">'.K_NEWLINE;
echo '<div class="pbox txt-left bd-gray3 mb-20 bdl-5-gray3"><p><span class="ft-bold">Generate Halaman Ujian</span> akan meningkatkan kinerja secara signifikan pada saat proses mulai ujian. Manfaatnya akan lebih terasa apabila Ujian dilakukan oleh banyak peserta dalam waktu yang bersamaan.</p>'.K_NEWLINE;
echo '<p class="ft-bold">Catatan:</p>'.K_NEWLINE;
echo '<ol><li>Proses Generate Halaman Ujian merupakan langkah OPSIONAL (TIDAK WAJIB);</li><li>Soal yang digenerate melalui halaman ini juga dapat dikerjakan secara offline oleh peserta ujian;</li><li>Setelah proses generate selesai, Ujian yang akan dikerjakan pada daftar test akan berwarna <span class="ft-fuchsia ft-bold">FUCHSIA (PINK TUA)</span>;</li><li><span class="pwrap bg-yellow ft-white ft-bold">Peringatan</span> Proses generate halaman ujian online mungkin akan memakan waktu cukup lama tergantung seberapa banyak peserta, jumlah soal/opsi/topik yang dimasukkan ke dalam test.</li></ol>'.K_NEWLINE;
echo '</div>'.K_NEWLINE;
...
//echo getFormRowTextInput('new_test_password', $l['w_password'].' (Token)', $l['h_test_password'], '', $test_password, $test_password, 20, false, false, false);
echo getFormRowTextInput('new_test_password', $l['w_password'].' / Token'.$startTag.$test_password.$endTag, $l['h_test_password'], ' ('.$l['d_password_lenght'].') ', '', K_USRREG_PASSWORD_RE, 255, false, false, true);

echo '<div class="row">'.K_NEWLINE;
echo '<br />'.K_NEWLINE;
echo '<input type="hidden" name="test_password" id="test_password" value="'.$test_password.'" />'.K_NEWLINE;
//echo '<input type="text" name="test_password" id="test_password" value="'.$test_password.'" />'.K_NEWLINE;

// show buttons by case


if (isset($test_id) and ($test_id > 0)) {
    echo '<span class="cbwrap">';
...
echo '<input type="submit" name="clear" id="clear2" value="Kosong" title="Kosongkan Pengisian Formulir">';
...
//        echo '</div>'.K_NEWLINE;
	echo $hide_tag_start;
        echo '<div class="d-block txt-left">'.K_NEWLINE;
        echo '<span class="label d-iblock w-137 mr-5">&nbsp;&nbsp;&nbsp;</span><span class="formw"><a href="#generate_test_link" name="generate_test_link" id="generate_test_link" title="Generate Test Page" class="xmlbutton mt-1em"><i class="fa fa-history"></i> Generate Halaman Ujian</a></span>';
		echo '<p id="gen_start_p" class="hidden"><span class="label d-iblock w-137">Generate Start Time </span><span id="gen_start" class="d-iblock pwrap bg-hijau ft-white p-3 formw"></span></p>';
		echo '<p id="gen_end_p" class="hidden"><span class="label d-iblock w-137">Generate End Time </span><span id="gen_end" class="d-iblock pwrap bg-biru ft-white p-3 formw"></span></p>';
		echo '<p id="gen_end"></p>';
        echo '</div>'.K_NEWLINE;
...
**/
	echo 'function checkTotalUser(){'.K_NEWLINE;
	    echo '$.ajax({'.K_NEWLINE;
	    //echo '		\'url\': \'/cbt/public/code/tce_test_execute.php\','.K_NEWLINE;
	    echo '		\'url\': \'tmf_generate_test.php?check_total_user&test_id='.$test_id.'\','.K_NEWLINE;
	    echo '		\'type\': \'GET\','.K_NEWLINE;
	    //echo '		\'data\': $(\'form#testform\').serialize(),'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("img#loaderSimpan").fadeIn("fast")},'.K_NEWLINE;
	    echo '		\'success\': function(result){$("div#generated").css({"padding":"7px 14px","color":"#fff","background":"#4caf50","borderRadius":"5px","marginBottom":"25px"});$("div#generated span#total_user").html("Sistem sedang melakukan generate halaman ujian untuk : "+result+" peserta.")}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("img#loaderSimpan").show().prepend("saved")}'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("input").prop("disabled",true)},'.K_NEWLINE;
	    //echo '		\'success\': function(result){$("input").removeAttr("disabled")}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("span#tersimpan").fadeOut("slow")}'.K_NEWLINE;
	    echo '});'.K_NEWLINE;
	echo '}'.K_NEWLINE;
	...
	echo 'function generateTest(){	'.K_NEWLINE;
	    ...
	    //echo '		\'data\': $(\'form#testform\').serialize(),'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("img#loaderSimpan").fadeIn("fast")},'.K_NEWLINE;
	    echo '		\'beforeSend\': function(){$("div#generated").css({"padding":"7px 14px","color":"#fff","background":"yellow","borderRadius":"5px","marginBottom":"25px"});$("div#generated span#done").html("<b><span class=\'d-iblock pwrap bg-fuchsia mt-10 bd-white boxshd\'>&infin; LOADING . . . </span></b>")},'.K_NEWLINE;
	    echo '		\'success\': function(result){$("div#generated").css({"padding":"7px 14px","color":"#fff","background":"#4caf50","borderRadius":"5px","marginBottom":"25px"});$("div#generated span#done").html("<b><span class=\'d-iblock mt-10 ft-white\'>&check; SELESAI<br/>"+result+"</span></b>");$("p#gen_end_p").show();$("span#gen_end").text($("input#timer").val());$("a#generate_test_link").hide();alert("Selesai melakukan generate test data.");location.reload()}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("img#loaderSimpan").show().prepend("saved")}'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("input").prop("disabled",true)},'.K_NEWLINE;
	    //echo '		\'success\': function(result){$("input").removeAttr("disabled")}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("span#tersimpan").fadeOut("slow")}'.K_NEWLINE;
	...
	echo 'function delGenTest(){	'.K_NEWLINE;
	    ...
	    echo '		\'success\': function(result){alert("Selesai menghapus data test");location.reload();},'.K_NEWLINE;
	    echo '});'.K_NEWLINE;
	echo '}'.K_NEWLINE;


	echo 'function checkGeneratedTest(){	'.K_NEWLINE;
		//echo 'alert("xxx")'.K_NEWLINE;
		echo '$.ajax({'.K_NEWLINE;
	    //echo '		\'url\': \'/cbt/public/code/tce_test_execute.php\','.K_NEWLINE;
	    ...
	    //echo '		\'data\': $(\'form#testform\').serialize(),'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("img#loaderSimpan").fadeIn("fast")},'.K_NEWLINE;
	    echo '		\'success\': function(result){$("div#generated").css({"padding":"7px 14px","color":"#fff","background":"#4caf50","borderRadius":"5px","marginBottom":"25px"});$("div#generated span#generated_user").html("sudah tergenerate sebanyak : "+result+" peserta.")}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("img#loaderSimpan").show().prepend("saved")}'.K_NEWLINE;
	    //echo '		\'beforeSend\': function(){$("input").prop("disabled",true)},'.K_NEWLINE;
	    //echo '		\'success\': function(result){$("input").removeAttr("disabled")}'.K_NEWLINE;
	    //echo '		\'complete\': function(result){$("span#tersimpan").fadeOut("slow")}'.K_NEWLINE;
