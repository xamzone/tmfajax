...
function F_show_select_test($order_field, $orderdir, $firstrow, $rowsperpage, $andwhere = '', $searchterms = '')
{
    ...
    if ($r = F_db_query($sql, $db)) {
        if ($m = F_db_fetch_array($r)) {
            ...
            echo '<input type="radio" name="checkall" id="checkall1" value="1" onclick="checkAll(\'#form_testselect td input[type=checkbox]\')" />';
            echo '<label for="checkall1">'.$l['w_check_all'].'</label> ';
            echo '<input type="radio" name="checkall" id="checkall0" value="0" onclick="unCheckAll(\'#form_testselect td input[type=checkbox]\')" />';
